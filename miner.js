...
class KaleidoMiningBot {
    ...
    async updateBalance(finalUpdate = false) {
        try {
            const newEarnings = this.calculateEarnings() || 0;
            const payload = {
                wallet: this.wallet,
                earnings: {
                    total: (this.currentEarnings.total || 0) + newEarnings,
                    session: newEarnings,  
            ...
            if (response.data.success) {
                this.currentEarnings.total = response.data.balance || (this.currentEarnings.total + newEarnings);
                this.currentEarnings.session = newEarnings;  
    ...
    logStatus(final = false) {
        ...
        Total: ${chalk.cyan((this.currentEarnings.total || 0).toFixed(8))} KLDO
        Pending: ${chalk.yellow((this.currentEarnings.pending || 0).toFixed(8))} KLDO
        Paid: ${chalk.green((this.currentEarnings.paid || 0).toFixed(8))} KLDO
        Referral Bonus: ${chalk.magenta(`+${((this.referralBonus || 0) * 100).toFixed(1)}%`)}
        `));
    }
